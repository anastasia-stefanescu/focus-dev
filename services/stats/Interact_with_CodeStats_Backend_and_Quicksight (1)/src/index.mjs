...
const createQuicksightDataset = async() => {
  const createDatasetParams = {
    ...
    PhysicalTableMap: { // Example of a simple physical table mapping using an S3 source
      'StringKey': {  // This key can be any string, but must be unique within PhysicalTableMap
        S3Source: {
          DataSourceArn: 'arn:aws:quicksight:us-east-1:123456789012:datasource/your-datasource-id',
          InputColumns: [
            {
              Name: 'column_name',
              Type: 'STRING'  // Types can be STRING, INTEGER, DECIMAL, DATETIME, etc.
            }
